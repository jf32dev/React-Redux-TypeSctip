import { EEntityType, Tab, Channel } from '@redbull/services';
...
export const getPrivateTab = async () => {
  ...
  if (!tabs.hasError) {
    return tabs.value.find((tab: any) => tab.isPersonal) || null;
  }
...
export const getMyChannel = async (personalTabId: number) => {
  ...
  if (!channels.hasError) {
    return (
      channels.value.find((channel: any) => channel.name === MY_CHANNEL) || null
    );
  }
